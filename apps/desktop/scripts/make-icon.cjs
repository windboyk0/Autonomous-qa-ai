 ...
 *
 * **CJS 여야 한다.** `.mjs` 로 두면 Electron 이 이것을 자기 모듈 시스템 밖에서
 * 읽어, `electron` 이 API 가 아니라 실행 파일 경로 문자열로 잡힌다
 * (`nativeImage` 가 undefined 가 된다). 패키지가 `"type": "module"` 이라
 * 확장자로 CJS 임을 밝혀야 한다.
 *
...
const outIco = resolve(__dirname, "..", "build", "icon.ico");
const outPng = resolve(__dirname, "..", "build", "icon.png");
