...
- [ ] **Project Scanner Agent** — 기술 스택 판별 (package.json / build.gradle / pom.xml / requirements.txt / docker-compose.yml)
- [ ] **Source Scanner** — `.gitignore` 존중, `node_modules`·빌드 산출물 제외, 파일 크기 상한
- [ ] **Security Analyzer Agent** — 권한 검사 누락 (직접 구현하는 유일한 검출기)
- [ ] 외부 도구 오케스트레이션 — ESLint / tsc --noEmit / Semgrep 이 **있으면** 실행하고 결과를 후보로 변환
- [ ] `IssueCandidate.source = "source"` + `codeRefs` 계약 추가

**DoD**: 정답지 탐지율 80% 이상, **오탐 3건 이하**. 실행 QA 결과가 손실되지 않는다.
