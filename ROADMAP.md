...
- [x] **Change Impact** (`source/change-impact.ts`) — git → 엔드포인트 → 힌트
- [x] 탐색 큐 순서 조정 (`explorer.takeNextPlan`)
- [x] 리포트 §3-1 "이번 변경이 닿는 영역"

**DoD 결과 — 통과 (테스트 302건 누계)**

| 검증 | 결과 |
|---|---|
| 커밋하지 않은 변경 우선, 깨끗하면 마지막 커밋 | 통과 |
| 컨트롤러 변경 → 엔드포인트 확정 | 통과 |
| 서비스 변경 → 참조 컨트롤러로 확장(추정 명시) | 통과 |
| 연결을 못 찾으면 그 사실을 적는다 | 통과 |
| 닿는 화면이 더 높은 점수 | 통과 |

### Phase 10에서 확정된 설계

- **순서만 바꾸고 대상은 줄이지 않는다.** 영향 분석은 추정이다. 범위를 줄이면
  못 본 화면이 생기는데 그 사실이 리포트에서 "정상"으로 읽힌다. 점수가 0인 화면도
  예산이 남는 한 전부 본다.
- **점수가 같으면 넣은 순서를 지킨다.** 여기서 순서가 흔들리면 Phase 2의
  "2회 실행 결과 동일" 이 깨져 회귀 테스트가 불가능해진다.
- **호출 그래프를 만들지 않는다.** 컨트롤러는 어노테이션으로 확정하고, 서비스가
  바뀌었을 때는 **그 클래스 이름을 언급하는 컨트롤러**를 찾는 데까지만 간다.
  그리고 그것이 추정이라는 사실을 근거로 남긴다.
- **`git status --porcelain` 은 줄을 trim 하기 전에 잘라야 한다.** 형식이 `XY path` 인데
  스테이징하지 않은 수정은 X 가 공백이라, 먼저 trim 하면 경로가 한 글자 깎인다(실측).
...
- [x] **API-Source Mapper** (`source/api-map.ts`) — 엔드포인트 → 컨트롤러 파일·줄
- [x] `apiRef` 계약 — 제목 파싱 대신 관측 사실을 필드로 들고 다닌다
- [x] 통합 모드에서 실행 결함에 소스 위치를 붙인다 (AI 보강 **전에**)
- [x] 매핑 실패를 미검증 목록에 남긴다

**DoD 결과 — 통과 (테스트 314건 누계)**

| 검증 | 결과 |
|---|---|
| 엔드포인트 5건이 컨트롤러 정확한 줄로 | 5/5 |
| 실제 id 경로(`/api/users/1204`) 정규화 연결 | 통과 |
| 서비스는 추정(confidence 0.5)·줄 번호 없음 | 통과 |
| 모르는 경로에 억지 매핑 없음 | 통과 |
| 통합 모드가 한 리포트에 실행+소스 결함 | 통과 |

### Phase 11에서 확정된 설계

- **제목을 파싱하지 않는다.** `HTTP 500: GET /api/x` 를 정규식으로 뜯으면 제목을
  한 번 다듬을 때마다 매핑이 조용히 깨진다. 관측한 사실(`apiRef`)을 필드로 들고 다닌다.
- **호출 그래프를 만들지 않는다.** 컨트롤러는 어노테이션으로 confidence 1.0 확정.
  그 컨트롤러가 **주입받는 타입**은 confidence 0.5 로 **파일만** 가리키고 줄은 비운다.
  아는 척하는 줄 번호보다 없는 줄 번호가 낫다.
- **못 찾으면 비운다.** 비슷한 것을 억지로 붙이는 순간 개발자가 엉뚱한 파일을 열고,
  그때 이 리포트의 신뢰가 끝난다. 대신 못 찾은 요청을 미검증 목록에 남긴다.
- **매핑은 AI 보강보다 먼저.** 모델에게 소스 위치를 알려주고 원인을 묻는 것과
  모르는 채로 묻는 것은 답의 질이 다르다.
- **fixture-admin 과 fixture-src 는 일부러 다른 앱이다.** 실무에서도 소스 경로를
  잘못 지정하는 일이 흔하고, 그때 도구가 "연결됨"처럼 굴면 안 된다.
  통합 E2E 는 **매핑이 실패하고 그 사실이 리포트에 남는 것**을 검증한다.

### 남은 것

- **Express / Next.js 라우트 매핑.** 지금은 Spring 만 다룬다. Node 백엔드를
  지원하려면 별도 fixture 가 필요하다.
...
- [x] **Build/Test Agent** (`source/build-test.ts`) — 허용 목록 기반 실행
- [x] `allowBuild` / `allowTest` 동의 (기본 전부 false)
- [x] 실패한 명령 → 결함 후보, 실행하지 않은 명령 → 미검증 목록
- [x] Electron 에 실행 동의와 경고

**DoD 결과 — 통과 (테스트 322건 누계)**

| 검증 | 결과 |
|---|---|
| 기본값에서 실행 계획 | **비어 있음** |
| 허용 목록 밖 script(`npm run deploy`) | 발견해도 실행 안 함 |
| 임의 명령 문자열(`... && echo pwned`) | 거부 |
| 의존성 설치 | 계획에 아예 없음 |
| 빌드 동의가 테스트를 실행시키는가 | 아니오 |
| 실패한 빌드 → 결함(종료 코드 포함) | 통과 |

### Phase 12에서 확정된 설계

- **Windows 에는 진짜 샌드박스가 없다.** 그러므로 통제 수단은 격리가 아니라 셋이다.
  ① 동의 없이는 아무것도 실행하지 않는다 ② 실행할 명령을 문자 그대로 보여준다
  ③ 허용된 **모양**의 명령만 실행한다. 이 한계를 UI 에서 숨기지 않는다.
- **의존성 설치는 동의가 있어도 하지 않는다.** 설치는 실행보다 넓은 권한이고,
  QA 도구가 남의 프로젝트에 파일을 쓸 이유가 없다.
- **Node 20+ 는 Windows 에서 `.cmd` 를 직접 spawn 하지 못한다**(보안 수정 후 EINVAL).
  그래서 npm·mvn 은 cmd.exe 를 거친다. 셸을 거치는 것 자체가 위험한 게 아니라
  **신뢰할 수 없는 문자열을 셸에 넘기는 것**이 위험하며, 여기서 넘기는 문자열에는
  사용자 입력이 한 글자도 없다(상수 + `build|test|lint` 세 낱말).
- **정적 분석이 끝난 뒤에 실행한다.** 명령이 프로젝트 파일을 바꿀 수 있고,
...
## 1.0 — 실제 코드베이스에 돌려 본 것

Phase 12 를 마친 뒤 **이 저장소 자체**에 소스 QA 를 돌렸다. fixture 만으로는
안 보이는 것이 두 개 나왔다.

| 증상 | 원인 | 조치 |
|---|---|---|
| 스캔 파일 **1049개** — 동봉 Chromium 소스까지 읽음 | `.gitignore` 를 존중하지 않음 | `git ls-files -co --exclude-standard` 로 목록을 받는다 |
| `log("warn", "UPDATE 건너뜀 — " + marker)` 가 SQL 주입으로 잡힘 | SQL 키워드로 **시작만** 해도 매치 | FROM·WHERE 같은 절 키워드까지 요구 |

고친 뒤 스캔 파일이 **151개**로 줄었고, 남은 결함은 전부 fixture(일부러 심은 것)와
테스트 파일의 자격증명 리터럴이었다.

**`.gitignore` 를 직접 파싱하지 않는다.** 부정 패턴·디렉터리 한정·중첩 규칙이
생각보다 복잡하고, 틀리면 조용히 잘못된 범위를 읽는다. git 에게 물으면 정확하다.
Git 저장소가 아니면 직접 훑되 **"무시 규칙을 적용하지 못했다"를 리포트에 적는다.**

---

## 1.0.1 — Claude Max 가 붙지 않던 문제

사용자가 Claude Max 를 골랐는데 "AI 사용 안 함" 으로 QA 가 돌았다. 원인이 둘이었다.

**1. `claude` 가 PATH 에 없었다.** 설치는 되어 있었고 터미널에서는 동작했다.

```
PATH 에 claude 없음
실제 위치: C:Users<user>.localinclaude.exe
```

Electron 앱은 로그인 셸을 거치지 않아 사용자 터미널의 PATH 와 다르다 —
**터미널에서는 되는데 앱에서는 안 되는 상황이 정상적으로 생긴다.**
알려진 설치 위치까지 찾고, 그래도 없으면 `QA_CLAUDE_PATH` 로 지정할 수 있게 했다.
못 찾았을 때는 **어디를 찾아봤는지** 그대로 보여준다.

**2. AI 상태를 탐색이 끝난 뒤에야 알렸다.** 그래서 탐색 내내 "사용 안 함" 으로 보였고,
연결 실패도 4분짜리 탐색이 끝난 뒤에 알았다. 이제 **탐색 전에** 확인하고 알린다.
화면도 모르는 것을 "사용 안 함" 이라고 쓰지 않고 "확인 중…" 으로 쓴다.

곁들여 고친 것:

- **프롬프트를 stdin 으로 넘긴다.** 인자로 넘기면 Windows 명령줄 상한(약 32KB)에
  걸리고, 대상 사이트에서 온 텍스트가 따옴표 하나로 인자 경계를 무너뜨린다.
- **아이콘.** `signAndEditExecutable: false` 가 서명뿐 아니라 **아이콘 삽입까지**
  막고 있었다. 인증서가 없으면 electron-builder 가 알아서 서명을 건너뛰므로
  그 스위치는 필요 없다.
...
`mobile/` 에 있던 손수 돌리던 하네스(explore.py + Claude Code)를 도구 안으로 들인다.

## 왜 싸게 붙나

구조가 웹 엔진과 거의 같다. **전송 계층만 바꿔 끼우는 일**이다.

| 웹 | 앱 |
|---|---|
| Playwright `Page` | adb shell |
| DOM/ARIA | uiautomator 덤프 |
| `discoverActions` | clickable 노드 |
| Collector(console/network) | logcat |
| `captureScreen` | screencap |
| judge · report · Monitor · 증적 | **그대로** |

## 하지 않는 것 — 미리 정한 한계

- **iOS 는 대상이 아니다.** adb 는 Android 전용이고 iOS 는 전송 계층이 통째로 다르다.
- **한글 입력은 초기 범위 밖.** `adb shell input text` 는 ASCII 만 보낸다.
  폼 채우기는 별도 IME 가 필요하므로 앱 QA 는 **Read 전용**으로 시작한다.
- **앱의 네트워크는 못 본다.** 프록시 없이는 요청을 볼 수 없다. logcat 에 찍히는 것만 본다.
- **결정성 DoD 를 완화한다.** 기기 상태(GPS·시간·데이터)가 매번 달라 웹의
  "2회 실행 결과 동일"은 성립하지 않는다.

## Phase 13 — 착지장 (adb · preflight · 설정) ✅

- [x] `app/adb.ts` — 전송 계층. explore.py 를 TypeScript 로 옮겼다
- [x] `app/observe.ts` — 덤프 → 요소 + 화면 지문
- [x] `app/preflight.ts` — adb·기기·앱 설치를 **단계별로** 확인
- [x] adb 동봉 (`scripts/fetch-adb.mjs` + 스테이징, 8MB)
- [x] `mode: "app"` 계약, CLI `--package/--device/--adb`, 설정 화면

**DoD 결과 — 통과 (테스트 369건 누계)**

| 검증 | 결과 |
|---|---|
| 동봉 adb 로 기기 목록 조회 | 통과 |
| adb 없음·기기 없음·미허용·앱 미설치를 **각각 다른 안내**로 | 통과 |
| 텍스트가 달라도 같은 화면, 구조가 다르면 다른 화면 | 통과 |
| 덤프 실패와 빈 화면을 구분 | 통과 |
| 패키지명 없으면 시작 거부 | 통과 |

### Phase 13에서 확정된 설계

- **Python 을 요구하지 않는다.** `explore.py` 를 그대로 실행하는 편이 빨랐겠지만,
  설치본이 Python 을 필요로 하면 "설치 후 5분 내 QA 시작"(§29)이 깨진다.
- **adb 를 동봉한다.** Chromium 을 동봉한 것과 같은 이유다. 8MB 라 설치본 크기에
  사실상 영향이 없고, 사용자가 SDK 를 깔 이유가 없어진다.
- **라벨로 방문을 기록하지 않는다.** `explore.py` 는 눌러 본 라벨 문자열로 중복을
  막았는데, 그러면 "확인"·"저장" 같은 흔한 라벨이 있는 다른 화면을 통째로 건너뛴다.
  **구조 지문**을 쓴다 — 웹의 `structureHash` 와 같은 원리다.
- **기기가 여러 대면 아무거나 고르지 않는다.** 남의 기기에서 QA 가 돌면 되돌릴 수 없다.
- **되돌릴 수 없는 동작은 기본으로 시도하지 않는다.** 출근하기·퇴근하기는 실제 근태
  기록을 남긴다. 웹의 삭제 동의와 같은 성격이라 별도 동의를 받는다.

## Phase 14 — 자율 탐색 루프 ✅

- [x] `app/driver.ts` — 조작 계약. 기기 없이도 루프를 검증하기 위해 인터페이스로 분리
- [x] `app/explorer.ts` — 깊이 우선 + back 되돌아오기 + 경로 재생
- [x] `app/classify-app.ts` — 앱 전용 위험도 판정
- [x] 예산(스텝·화면·시간), 이탈 복구, 스크롤로 숨은 요소 찾기

**DoD 결과 — 통과 (테스트 381건 누계)**

| 검증 | 결과 |
|---|---|
| 도달 가능한 화면을 모두 방문 | 4/4 |
| 같은 화면에서 같은 요소를 두 번 누르지 않음 | 통과 |
| 죽은 버튼(눌러도 안 바뀜)을 기록 | 통과 |
| 앱 밖으로 나가면 다시 띄우고 이어감 | 통과 |
| 되돌릴 수 없는 동작은 동의 없이 안 누름 | 통과 |
| 동의해도 **맨 마지막에** 시도 | 통과 |
| 예산에서 멈추고 사유를 남김 | 통과 |

### Phase 14에서 확정된 설계

- **드라이버를 인터페이스로 뺐다.** 실제 기기로는 루프를 잴 수 없다 — 기기 상태가
  매번 달라 같은 결과가 안 나온다. 화면 그래프를 흉내 내는 가짜 기기로
  "규칙대로 도는가"만 정확히 잰다. 웹이 fixture 서버를 쓴 자리와 같다.
- **웹처럼 큐를 쓰지 않는다.** 앱에는 주소가 없어 아무 화면으로 바로 갈 수 없다.
  깊이 우선으로 들어갔다가 back 으로 나오고, 못 나오면 앱을 다시 띄워 경로를 재생한다.
- **지문에 제목과 라벨을 넣는다.** 클래스·resource-id 만 넣었더니 **모든 화면이 같은
  지문**이 됐다 — Flutter 덤프는 대부분 `android.view.View` 에 id 도 없다(실측: 4개 화면이
  1개로 보였다). 웹이 heading·navLabels 를 넣는 것과 같은 이유다. 다만 같은 모양이
  3번 이상 반복되면 목록으로 보고 라벨을 버린다 — 47명이 47개 화면이 되면 안 된다.
- **모르는 라벨은 이동으로 본다.** 웹은 모르면 CAUTION 으로 두고 안 누르지만, 앱에는
  href 같은 다른 신호가 없어 그러면 화면을 거의 못 본다(실측: 4개 중 1개만 눌렸다).
  대신 denylist 와 쓰기 라벨은 그대로 막고, 무엇을 눌렀는지 전부 기록한다.
- **되돌릴 수 없는 라벨은 사용자가 정한다.** 도구는 남의 앱 도메인을 모른다 —
  "출근하기"는 라벨만 보면 평범하지만 실제로는 근태 기록을 남긴다.
  흔한 것을 기본으로 두고 자기 앱에 맞게 더할 수 있게 했다.
## Phase 15 — 이상 검출 → 리포트 (예정)
## Phase 16 — 위험 액션 정책과 예산 (예정)
