...
**대상을 줄이지 않고 순서만 바꾼다** — 추정이 틀려도 잃는 것은 순서뿐이다.

### Program Scope QA — 프로그램 범위 한정 (4부, §32~33)

Change Impact와 같은 계열의 범위 한정기이지만 성격이 다르다. Change Impact는 git diff를
입력으로 **순서만** 바꾸는 반면, Program Scope는 사용자가 준 **소스 파일 목록**을 입력으로
받아 연관된 화면**만** 탐색하도록 **범위 자체를 줄인다**. "부분 검수" 기능이 바로 이것이다.
자세한 내용은 §32(API 검증)·§33(프로그램 범위 한정)을 참고한다.
...
4부(API 검증 + 프로그램 범위)에서 추가되는 에이전트:

| name | 담당 |
|---|---|
| `qa-api-verify` | OpenAPI/manual 케이스 직접 호출, 상태코드·스키마 검증, 안전 정책(§32) |
| `qa-program-scope` | 소스 파일 목록 → 엔드포인트 → 화면 매핑, Explorer 범위 시드 생성(§33) |
...
## 32. API 검증 (4부, Phase 17)

`analyResult.md` §0-3이 지적한 편차 — "API를 화면 조작이 유발한 요청으로만 관측한다" —
를 메운다. 화면을 거치지 않고 엔드포인트를 **직접 호출**해 상태코드·응답 스키마·응답
시간을 검증한다. 상세 설계는 `vibeContext/apiVerifyAndProgramScope.md` §1.

- **기본은 꺼짐**(`apiVerify.enabled: false`). 꺼지면 기존 파이프라인과 완전히 동일하다.
- **저장소를 뒤져 엔드포인트를 추측하지 않는다.** OpenAPI 스펙(`specPath`) 또는
  사용자가 등록한 `manualCases`에 있는 것만 호출한다 (§16-1과 같은 결).
- **실행 시점**: 로그인 성공 직후, Explorer 생성 이전 — 탐색이 세션을 흔들기 전.
- **인증**: 기본은 `reuse-session`(Playwright `context.request`로 로그인 쿠키 재사용).
  로그인이 꺼져 있거나 실패하면 이 모드는 건너뛰고 사유를 리포트에 남긴다.
  `bearer`(토큰, `registerSecret()`으로 마스킹)·`none`(공개 API)도 지원한다.

### 안전 정책 — 쓰기 메서드는 기본적으로 실행하지 않는다

CRUD 데이터 소유권(§10)과 같은 원칙을 API 직접 호출에도 적용한다.

| 메서드 | 기본 | 실행 조건 |
|---|---|---|
| GET / HEAD | 자동 허용 | 항상 |
| POST / PUT / PATCH | 계획만 생성, 실행 안 함 | `allowWriteMethods=true` **그리고** 요청에 `AUTO-QA-` 마커가 있을 때만 |
| DELETE | 차단 | 위 조건 + `crud.deleteConsent`까지 전부 있을 때만 |

마커를 넣을 수 없는 쓰기 케이스는 억지로 실행하지 않고 "미검증(쓰기 API, 안전한
테스트 데이터 생성 불가)"으로 리포트에 남긴다.

### 응답 검증과 §16 원칙의 절충

§16은 응답 본문을 저장하지 않는다고 못박았지만, 스키마 검증은 본문을 보지 않고는
할 수 없다. 절충안: **검증에 성공한 필드는 값이 아니라 이름·타입만** 증적에 남기고,
**실패했을 때만** 마스킹된 값 일부를 남긴다. 스키마 검증은 필수 필드·타입만 보고
`additionalProperties`는 항상 허용한다 — 모르는 필드로 실패시키면 오탐이 쏟아진다.

새 `ruleId`: `API-STATUS-MISMATCH` · `API-SCHEMA-INVALID` · `API-TIMEOUT` ·
`API-UNREACHABLE` · `API-AUTH-BYPASS`(비로그인 재호출로 잡는 런타임 권한 우회, 스펙에
`security`가 명시된 엔드포인트에 한함). 기존 `IssueCandidate.apiRef` 필드를 그대로
채워서 만들기 때문에, 통합 모드의 `attachCodeRefs()`가 수정 없이 이 결함에도 소스
위치를 붙여준다.

## 33. 프로그램 범위 한정 (4부, Phase 18~19)

`analyResult.md` §0-1이 지적한 편차 — "부분 검수 기능이 없다" — 를 메운다. 소스 파일
경로 목록을 입력하면 연관 API → 연관 화면을 추정해 **그 화면들만** 탐색한다. 상세
설계는 `vibeContext/apiVerifyAndProgramScope.md` §2.

- **기본은 꺼짐**(`programScope.enabled: false`). 꺼지면 Explorer는 기존과 완전히
  동일하게 전체 탐색한다 — 새 파라미터(`allowedScope`)가 `null`이면 아무것도 바뀌지
  않는다.
- **입력은 소스 파일 경로 목록**이다(업무 프로그램ID 체계와의 연결은 범위 밖).
  `programScope.sourceFiles` 또는 한 줄에 한 경로씩 적은 `sourceFilesListPath`.

### 파일 → 엔드포인트 → 화면

1. **파일 → 엔드포인트**: `source/change-impact.ts::explicitChangedFiles()`로 사용자
   목록을 `ChangedFiles`로 만들고, Change Impact(§업무10)의 `computeImpact()`를
   **그대로 재사용**한다. 새 매핑 로직이 없다.
2. **엔드포인트 → 화면**: 안 A(캐시 기반)를 채택했다. 과거 전체 탐색 Run의
   `graph.json` + `evidence.json`(화면별 network 증적)에서 "이 화면이 실제로 호출한
   엔드포인트"의 역인덱스를 만들고, 이번 변경의 엔드포인트와 대조한다.
   - confidence 3단계: 엔드포인트 정확 일치(1.0) · 경로만 일치(0.8) ·
     낱말 힌트 일치(0.5, 추정). `programScope.minConfidence`(기본 0.5) 미만은
     탐색 대상에서 빼고 미검증에만 남긴다.
   - **베이스라인이 없으면 매핑할 수 없다.** 이 경우 범위를 좁히지 않고 전체 탐색으로
     진행하며, 이 결과가 다음 부분 검수의 기준(베이스라인)이 된다는 사실을 리포트에
     명시한다. 매핑된 화면이 0개일 때도 같은 이유로 범위를 좁히지 않는다 — 빈 결과보다
     전체 탐색이 안전하다.
   - **호출 그래프를 만들지 않는다** (§11과 같은 원칙). 화면이 어느 API를 부르는지는
     브라우저가 실제로 방문해 관측한 사실만 믿는다.

### Explorer 범위 제한

`Explorer`의 새 생성자 파라미터 `allowedScope`는 `impactHints`(§업무11, 순서만
바꾸는 비파괴적 힌트)와 다른 필드다 — 이것은 **범위 자체를 제한한다.**

- 큐를 매핑된 화면의 URL로만 초기화한다.
- 매핑된 화면 위에서 발견되는 액션(폼·버튼·모달·탭 등 같은 화면 안의 조작)은 기존과
  동일하게 전부 허용한다.
- 그 화면에서 **다른 새 화면으로 이어지는 링크**는 더 이상 확장하지 않는다(사유와
  함께 기록 — 조용히 넘어가지 않는다).

### 리포트

§3-1을 일반화해 §3-2 "검증 대상 프로그램 범위"로 확장했다: 입력 파일 목록 → 매핑된
엔드포인트(확정/추정 구분) → 매핑된 화면 → 매핑 실패 항목(미검증) 표. §3-3 "API 검증
결과"를 신설했다.

### 알려진 한계

- 컨트롤러 → 엔드포인트 매핑은 Spring 전용이다(§업무11과 같은 기존 한계).
  Express/Next.js 백엔드는 1단계 범위 밖이다.
- Electron 화면(파일 여러 개 선택 다이얼로그 등)은 아직 연결하지 않았다 — 엔진
  CLI/설정 스키마까지가 이번 범위다("엔진 우선, UI 최후" 원칙, §27 원칙 A).
