...
export function preflightApp(input: {
  ...
  /*
   * 기기를 고르지 않았는데 여러 대가 붙어 있으면 **아무거나 고르지 않는다.**
   * 남의 기기에서 QA 가 돌면 되돌릴 수 없다.
   */
  const chosen =
    input.deviceSerial.trim() !== ""
      ? (ready.find((d) => d.serial === input.deviceSerial) ?? null)
      : ready.length === 1
        ? ready[0]!
        : null;
    ...
    return {
      ...
      detail:
        input.deviceSerial.trim() !== ""
          ...
          : `기기가 ${ready.length}대 연결되어 있습니다. 어느 것을 쓸지 골라야 합니다.`,
      remediation: `연결된 기기: ${ready.map((d) => `${d.serial}${d.model ? ` (${d.model})` : ""}`).join(", ")}`,
    };
  }
