...
Phase 2 DoD의 채점 기준. **SAFE 액션만으로 도달 가능한 상태 16개**다.
(로그인 화면을 더하면 증적이 남는 화면은 17개. 로그인은 탐색 전에 처리되므로 그래프에는 없다)
...
| 15 | 부서관리 | `/depts` | — | 사이드바 |
| 16 | 부서관리 > 부서 등록 | `/depts` | dialog `부서 등록` | 목록 > + 새 부서 (버튼) |

**#16은 주소가 없는 등록 화면이다.** 진입점이 `<a href>` 가 아니라 `<button>` 이고
눌러도 주소가 그대로다. 실제 SPA 관리자웹의 흔한 모양이며, 이 화면이 없으면
"등록 화면을 찾지 못했습니다" 로 Create·Update·Delete 가 전부 미실행이 되는 회귀를
잡을 수 없다 — 실측에서 그렇게 끝난 적이 있다.
...
Phase 6에서 `설문 수정` 화면이 추가되며 13 → 14, 모달 등록을 재현하는 `부서관리` 와
그 등록 모달이 추가되며 14 → 16으로 늘었다.
