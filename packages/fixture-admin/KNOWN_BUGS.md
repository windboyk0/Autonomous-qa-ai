...
| Phase 6 (쓰기 테스트) | BUG-05·06 | **2/2 탐지** |

**정답지 10건 전부 탐지되었다.**

| 버그 | 룰 | 판정 |
|---|---|---|
| BUG-05 담당자 이메일 검증 누락 | `FUNC-VALIDATION-MISSING` | 필수 필드를 하나씩 비워 저장을 시도해 검출 |
| BUG-06 저장 버튼 무반응 | `FUNC-NO-RESPONSE` | 쓰기 요청도 화면 이동도 없어 Create FAIL |
